...
<script>
    export default {
        ...
        mounted() {
this.render_start_content();
        },
        methods: {
            test() {
            console.log(this.rezident);
            },


            render_start_content()
            {
                axios
                    .post('/get_subject',{
                    ...
                    .then(({ data }) => (
                    this.rezident=data[0].rezident,
                    this.status=data[0].status,
                    this.polnoe_naimenovanie=data[0].polnoe_naimenovanie,
                    this.socrasch_naimenovanie=data[0].socrasch_naimenovanie,
                    this.FIO=data[0].FIO,
                    this.mesto_nahozdenia=data[0].mesto_nahozdenia,
                    this.pochtovyi_adres=data[0].pochtovyi_adres,
                    this.razmer_ust_fonda=data[0].razmer_ust_fonda,
                    this.nomer_rasch_sheta=data[0].nomer_rasch_sheta,
                    this.naimenovanie_banka=data[0].naimenovanie_banka,
                    this.BIC=data[0].BIC,
                    this.vidy_tovarov=data[0].vidy_tovarov,
                    this.UNP=data[0].UNP,
                    this.balansovaja_stoim_aktivov=data[0].balansovaja_stoim_aktivov,
                    this.objem_viruchki=data[0].objem_viruchki,
                    this.dominant=data[0].dominant
                        )
...
if(this.alarm==false)
{
    axios
        ...
        }).then( Vue.router.push({name:'doc_subject', params: { notification_sub:true, notification_message:'Субъект изменён успешно'}}))
}
